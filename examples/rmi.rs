...
use fanuc_ucl::{ThreadConfig, joints::{JointFormat, JointTemplate}, rmi::{RmiDriver, RmiDriverConfig, proto}};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut driver = RmiDriver::new(RmiDriverConfig::default_with_ip([10, 0, 0, 1]));
    driver.connect(Some(ThreadConfig::new(80, None)))?;

    driver.send_full_reset()?.wait_timeout(Duration::from_secs(20))?;
    driver.send(proto::FrcInitialize::default())?.wait_timeout(Duration::from_secs(20))?;
    ...
    let pos_resp = driver.send(proto::FrcReadJointAngles::new(None))?.wait_timeout(Duration::from_millis(200))?;
